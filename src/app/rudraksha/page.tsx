...
export default function Rudraksha() {
  const [rudraksha, setGemstones] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const router = useRouter();

  const fetchGemstones = async () => {
    try {
      const res = await fetch("/api/rudraksha", {
        cache: "no-store",
      });
      ...
      console.error("Failed to fetch rudraksha:", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (item: any) => {
    router.push(`/rudraksha/${item._id}/edit`);
  };

  const handleToggleStatus = async (item: any) => {
    try {
      const res = await fetch(`/api/rudraksha/${item._id}/status`, {
        method: "PATCH",
        headers: {
  ...
  const handleDuplicate = async (item: any) => {
    try {
      const res = await fetch(`/api/rudraksha/${item._id}/duplicate`, {
        method: "POST",
      });
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to duplicate rudraksha");
      }

      fetchGemstones();
    } catch (error) {
      console.error(error);
      alert("Failed to duplicate rudraksha.");
    }
  };
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      const res = await fetch(`/api/rudraksha/${item._id}`, {
        method: "DELETE",
      });
      ...
      if (!res.ok) {
        throw new Error(data.message || "Failed to delete rudraksha");
      }

      fetchGemstones();
    } catch (error) {
      console.error(error);
      alert("Failed to delete rudraksha.");
    }
  };
  if (loading) {
    return (
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="mb-6 text-2xl font-bold">Rudraksha</h1>
        <button
          onClick={() => router.push("/rudraksha/create")}
